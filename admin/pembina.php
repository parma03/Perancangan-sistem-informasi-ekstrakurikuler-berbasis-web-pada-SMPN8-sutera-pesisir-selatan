...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tambah'])) {
    ...
    if ($stmt->execute()) {
        ...
        $query_admin = "INSERT INTO tb_pembina (id_user, pembina_nama, pembina_profile) VALUES (?, ?, ?)";
        ...
        if ($stmt_admin->execute()) {
            $stmt_admin->close();
            $stmt->close();
            $_SESSION['notification'] = "Data Pembina berhasil ditambah.";
            ...
            $stmt_admin->close();
            $stmt->close();
            $_SESSION['notification'] = "Gagal menambah data pembina.";
            $_SESSION['alert'] = "alert-danger";
            header("Location: pembina.php");
            exit();
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['edit'])) {
    ...
    $query_existing = "SELECT pembina_profile, username AS current_username FROM tb_pembina JOIN tb_user ON tb_pembina.id_user = tb_user.id_user WHERE tb_pembina.id_user = ?";
    ...
    $existing_profile = $existing_data['pembina_profile'];
    ...
    if ($stmt->execute()) {
        ...
        $query_admin = "UPDATE tb_pembina SET pembina_nama = ?, pembina_profile = ? WHERE id_user = ?";
        $stmt_admin = $conn->prepare($query_admin);
        ...
        if ($stmt_admin->execute()) {
            ...
            $stmt_admin->close();
            $stmt->close();
            $_SESSION['notification'] = "Data Pembina berhasil diupdate.";
            $_SESSION['alert'] = "alert-success";
            header("Location: pembina.php");
            exit();
        } else {
            ...
            $stmt_admin->close();
            $stmt->close();
            $_SESSION['notification'] = "Gagal update data pembina.";
            $_SESSION['alert'] = "alert-danger";
            header("Location: pembina.php");
            exit();
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
    ...
    $query_select = "SELECT pembina_profile FROM tb_pembina WHERE id_user = ?";
    $stmt_select = $conn->prepare($query_select);
    $stmt_select->bind_param("i", $id_user);
    $stmt_select->execute();
    $result = $stmt_select->get_result()->fetch_assoc();
    $profile = @$result['pembina_profile'];
    ...
    if ($stmt->execute()) {
        $_SESSION['notification'] = "Data Pembina berhasil dihapus.";
        $_SESSION['alert'] = "alert-success";
        header("Location: pembina.php");
        exit();
    } else {
...
$query = "SELECT * FROM tb_user INNER JOIN tb_pembina ON tb_user.id_user = tb_pembina.id_user";
...
<body class="g-sidenav-show bg-gray-100">
                    ...
                    <div class="card border-0 shadow-xs mb-4">
                        <div class="card-header border-bottom-0 pb-0">
                            <div class="d-sm-flex align-items-center mb-3">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Data Pembina</h6>
                                    <p class="text-sm text-muted mb-sm-0">
                                        Keseluruhan data User dengan Role Pembina
                                    </p>
                                </div>
                                <div class="ms-auto d-flex">
                                    <button type="button"
                                        ...
                                        <span class="btn-inner--text">Add Pembina</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center px-2 py-1">
                                                        <?php if ($data["pembina_profile"] === NULL) { ?>
                                                            <div class="trans-logo bg-gray-100 me-3">
                                                                <img src="../assets/img/team-2.jpg" class="w-75" alt="xd">
                                                            </div>
                                                        <?php } else { ?>
                                                            <div class="trans-logo bg-gray-100 me-3">
                                                                <img src="../assets/img/profile/<?php echo $data["pembina_profile"]; ?>"
                                                                    class="w-75" alt="xd">
                                                            </div>
                                                        <?php } ?>
                                                        <div>
                                                ...
                                                <td>
                                                    <p class="transaction-amount mb-0"><?php echo $data["pembina_nama"] ?>
                                                    </p>
                                                </td>
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-gradient-primary text-white">
                                                            <h5 class="modal-title"
                                                                ...
                                                                <i class="fas fa-user-circle me-2"></i>Detail Pembina
                                                            </h5>
                                                                    ...
                                                                    <div class="card border-0 shadow-sm">
                                                                        <div class="card-body">
                                                                            <h4 class="card-title mb-4 text-primary">
                                                                                <i class="fas fa-id-card me-2"></i>Pembina
                                                                                Information
                                                                            </h4>
                                                                            <div class="row mb-2">
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <center>
                                                            <div class="col-md-3">
                                                                ...
                                                                <?php } else { ?>
                                                                    <img class="card-img card-img-center align-items-center"
                                                                        src="../assets/img/profile/<?php echo $data["pembina_profile"]; ?>" />
                                                                <?php } ?>
                                                            </div>
                                                        </center>
                                                        <form action="pembina.php" method="POST"
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control" id="nama"
                                                                        name="nama"
                                                                        value="<?php echo $data["pembina_nama"] ?>"
                                                                        required>
                                                                </div>
                                                                <div class="mb-3">
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <p>Yakin Menghapus Data ini,<b>
                                                                    <?php echo $data['pembina_nama']; ?></b> ?</p>
    ...
    <div class="modal fade" id="modalAddAdmin" tabindex="-1" aria-labelledby="modalAddAdminLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalAddAdminLabel">Tambah Data Pembina</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="pembina.php" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        <div class="mb-3">
    ...
    <script>
        $(document).ready(function () {
            $('#datatable-transaksi').DataTable({
                ...
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search Data Pembina...",
                    paginate: {
